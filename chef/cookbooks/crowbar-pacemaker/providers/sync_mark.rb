...
action :wait do
  ...
  CrowbarPacemakerSynchronization.wait_for_mark_from_founder(node, mark, new_resource.fatal, new_resource.timeout)
end

action :create do
  action, mark = get_options(new_resource)
  CrowbarPacemakerSynchronization.set_mark_if_founder(node, mark)
end

action :sync do
  action, mark = get_options(new_resource)
  CrowbarPacemakerSynchronization.synchronize_on_mark(node, mark, new_resource.fatal, new_resource.timeout)
end
