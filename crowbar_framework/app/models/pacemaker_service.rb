...
class PacemakerService < ServiceObject
  ...
  def apply_role_pre_chef_call(old_role, role, all_nodes)
    ...
    role.default_attributes["pacemaker"]["drbd"] ||= {}

    if member_nodes.length == 2
      drbd_nodes = member_nodes.map(&:name)
    elsif old_role && old_role.default_attributes.key?("pacemaker")
      drbd_nodes = old_role.default_attributes["pacemaker"]["drbd"]["nodes"] || []
    else
      drbd_nodes = []
    end
    role.default_attributes["pacemaker"]["drbd"]["nodes"] = drbd_nodes
